...
//	m_tempCamera->SetLookingObject(m_Player);
//
...
bool GameScene::ProcessInput(HWND hWnd, POINT OldCursor, float ElapsedTime)
{
	...
	//{
	//	if (pKeyBuffer[KEY_W] & 0xF0) dwDirection |= DIR_FORWARD;
	//	if (pKeyBuffer[KEY_S] & 0xF0) dwDirection |= DIR_BACKWARD;
	//	if (pKeyBuffer[KEY_A] & 0xF0) dwDirection |= DIR_LEFT;
	//	if (pKeyBuffer[KEY_D] & 0xF0) dwDirection |= DIR_RIGHT;
	//	if (pKeyBuffer[KEY_Q] & 0xF0) {
	//		dwDirection |= DIR_UP;
	//	}
	//	if (pKeyBuffer[KEY_E] & 0xF0) {
	...
	//	// if (dwDirection) m_Player->Move(dwDirection, 50.0f * ElapsedTime, true);
	//}
