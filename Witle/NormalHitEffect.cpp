...
void NormalHitEffect::ReleaseObjects()
{
	if (m_NormalHitEffect)
	{
		m_NormalHitEffect->ReleaseObjects();
		delete m_NormalHitEffect;
		m_NormalHitEffect = nullptr;
	}
}

void NormalHitEffect::ReleaseUploadBuffers()
{
	if (m_NormalHitEffect) m_NormalHitEffect->ReleaseUploadBuffers();
}
	...
	: ComponentBase(pOwner)
{
	m_ResolX = 5;
	m_ResolY = 1;
	m_NormalHitEffect = new UI3DImage(pOwner, pd3dDevice, pd3dCommandList, POINT{ 0, 0 }, 100, 123,
...
void NormalHitEffect::Update(float fTimeElapsed)
{
	if (!isActive) return;
...
void NormalHitEffect::Render(ID3D12GraphicsCommandList * pd3dCommandList, const XMFLOAT3& pos)
{
	if (!isActive) return;
	...
	m_NormalHitEffect->Render(pd3dCommandList, uiWorld);
}
