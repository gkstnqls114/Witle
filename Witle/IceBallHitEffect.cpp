...
void IceBallHitEffect::ReleaseObjects()
{
	if (m_IceBallHitEffect)
	{
		m_IceBallHitEffect->ReleaseObjects();
		delete m_IceBallHitEffect;
		m_IceBallHitEffect = nullptr;
	}
}

void IceBallHitEffect::ReleaseUploadBuffers()
{
	if (m_IceBallHitEffect) m_IceBallHitEffect->ReleaseUploadBuffers();
}
	...
	: ComponentBase(pOwner)
{
	m_ResolX = 5;
	m_ResolY = 1;
	m_IceBallHitEffect = new UI3DImage(pOwner, pd3dDevice, pd3dCommandList, POINT{ 0, 0 }, 100, 123,
...
void IceBallHitEffect::Update(float fTimeElapsed)
{
	if (!isActive) return;
...
void IceBallHitEffect::Render(ID3D12GraphicsCommandList * pd3dCommandList, const XMFLOAT3& pos)
{
	if (!isActive) return;
	...
	m_IceBallHitEffect->Render(pd3dCommandList, uiWorld);
}
