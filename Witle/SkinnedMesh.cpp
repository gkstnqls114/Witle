...
LoadGameObject* LoadGameObject::LoadFrameHierarchyFromFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, ID3D12RootSignature * pd3dGraphicsRootSignature, LoadGameObject * pParent, FILE * pInFile)
{
	...
	for (; ; )
	{
		...
		{
			pGameObject = new LoadGameObject(nullptr);
		...
		{
			SkinnedMesh *pSkinnedMesh = new SkinnedMesh(nullptr, pd3dDevice, pd3dCommandList);
...
{
	if ((m_pSkinnedMesh && (m_pSkinnedMesh->GetType() & VERTEXT_BONE_INDEX_WEIGHT)) || (m_pStandardMesh && (m_pStandardMesh->GetType() & VERTEXT_BONE_INDEX_WEIGHT)))
	{
		SkinnedMesh *pSkinnedMesh = (SkinnedMesh *)m_pSkinnedMesh;
...
LoadGameObject * LoadGameObject::LoadGeometryAndAnimationFromFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, ID3D12RootSignature * pd3dGraphicsRootSignature, const char * pstrFileName, bool bHasAnimation)
{
	FILE *pInFile = NULL;
	::fopen_s(&pInFile, pstrFileName, "rb");
	::rewind(pInFile);

	LoadGameObject *pGameObject = LoadGameObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, NULL, pInFile);
...
void LoadGameObject::LoadMaterialsFromFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, LoadGameObject * pParent, FILE * pInFile)
{
	char pstrToken[64] = { '\0' };
...
void CMaterial::SetTexture(Texture * pTexture, UINT nTexture)
{
...
void CMaterial::LoadTextureFromFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, UINT nType, UINT nRootParameter, _TCHAR * pwstrTextureName, Texture ** ppTexture, LoadGameObject * pParent, FILE * pInFile)
{
	...
	if (strcmp(pstrTextureName, "null"))
	{
		...
		else
		{
			if (pParent)
			{
				...
				LoadGameObject *pRootGameObject = pParent;
