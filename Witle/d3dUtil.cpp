...
UINT d3dUtil::gnDsvDescriptorIncrementSize = 0;
...
D3D12_ROOT_PARAMETER d3dUtil::CreateRootParameterSRV(UINT ShaderRegister, UINT RegisterSpace, D3D12_SHADER_VISIBILITY ShaderVisibility)
{
	D3D12_ROOT_PARAMETER d3dRootParameter;
...
ID3D12Resource * d3dUtil::CreateTextureResourceFromWICFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, const wchar_t * pszFileName, ID3D12Resource ** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates)
{
	ID3D12Resource *pd3dTexture = NULL;
...
ID3D12Resource * d3dUtil::CreateTextureResourceFromFile(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, const wchar_t * pszFileName, ID3D12Resource ** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates)
{
	ID3D12Resource *pd3dTexture = NULL;
	std::unique_ptr<uint8_t[]> ddsData;
	std::vector<D3D12_SUBRESOURCE_DATA> vSubresources;
...
D3D12_SHADER_RESOURCE_VIEW_DESC d3dUtil::GetShaderResourceViewDesc(D3D12_RESOURCE_DESC d3dResourceDesc, UINT nTextureType)
{
...
D3D12_UNORDERED_ACCESS_VIEW_DESC d3dUtil::GetUnorderedAccessViewDesc(D3D12_RESOURCE_DESC d3dResourceDesc, UINT nTextureType)
{
	D3D12_UNORDERED_ACCESS_VIEW_DESC UnorderedAccessViewDesc;
	UnorderedAccessViewDesc.Format = d3dResourceDesc.Format; 
	switch (nTextureType)
	{
	case RESOURCE_TEXTURE2D: //(d3dResourceDesc.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE2D)(d3dResourceDesc.DepthOrArraySize == 1)
	case RESOURCE_TEXTURE2D_ARRAY:
	...
	case RESOURCE_BUFFER: //(d3dResourceDesc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER)
		...
		UnorderedAccessViewDesc.Buffer.NumElements = 0;
...
ID3D12Resource * d3dUtil::CreateTexture2DResource(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, UINT nWidth, UINT nHeight, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS d3dResourceFlags, D3D12_RESOURCE_STATES d3dResourceStates, D3D12_CLEAR_VALUE *pd3dClearValue)
{
	ID3D12Resource *pd3dTexture = NULL;
