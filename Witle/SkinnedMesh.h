...
private:
	LoadGameObject* LoadFrameHierarchyFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LoadGameObject *pParent, FILE *pInFile);
	void CacheSkinningBoneFrames(LoadGameObject* pRootFrame);
	void SetMesh(SkinnedMesh* mesh) { m_pMesh = mesh; };
...
public:
	...
	bool LoadGeometryAndAnimationFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, const char *pstrFileName, bool bHasAnimation);
