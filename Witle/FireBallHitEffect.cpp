...
void FireBallHitEffect::ReleaseObjects()
{
	if (m_FireBallHitEffect)
	{
		m_FireBallHitEffect->ReleaseObjects();
		delete m_FireBallHitEffect;
		m_FireBallHitEffect = nullptr;
	}
}

void FireBallHitEffect::ReleaseUploadBuffers()
{
	if (m_FireBallHitEffect) m_FireBallHitEffect->ReleaseUploadBuffers();
}
	...
	: ComponentBase(pOwner)
{
	m_ResolX = 5;
	m_ResolY = 1;
	m_FireBallHitEffect = new UI3DImage(pOwner, pd3dDevice, pd3dCommandList, POINT{ 0, 0 }, 100, 123,
...
void FireBallHitEffect::Update(float fTimeElapsed)
{
	if (!isActive) return;
...
void FireBallHitEffect::Render(ID3D12GraphicsCommandList * pd3dCommandList, const XMFLOAT3& pos)
{
	if (!isActive) return;
	...
	m_FireBallHitEffect->Render(pd3dCommandList, uiWorld);
}
