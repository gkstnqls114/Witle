...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	m_pd3dColorBuffer = d3dUtil::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf4Colors, sizeof(XMFLOAT4) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dColorUploadBuffer);
...
void CSkinnedMesh::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	if (m_pd3dcbBindPoseBoneOffsets)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(ROOTPARAMETER_SKINNEDBONEOFFSET, d3dcbBoneOffsetsGpuVirtualAddress); //Skinned Bone Offsets
	}
	...
	if (m_pd3dcbSkinningBoneTransforms)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(ROOTPARAMETER_SKINNEDBONETRANSFORM, d3dcbBoneTransformsGpuVirtualAddress); //Skinned Bone Transforms
	...
	for (; ; )
	{
		...
		else if (!strcmp(pstrToken, "<BoneOffsets>:"))
		{
			...
			if (m_nSkinningBones > 0)
			{
				...
				m_pd3dcbBindPoseBoneOffsets = d3dUtil::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
		...
		else if (!strcmp(pstrToken, "<BoneIndices>:"))
		{
			...
			if (m_nVertices > 0)
			{
				...
				m_pd3dBoneIndexBuffer = d3dUtil::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmn4BoneIndices, sizeof(XMINT4) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dBoneIndexUploadBuffer);
		...
		else if (!strcmp(pstrToken, "<BoneWeights>:"))
		{
			...
			if (m_nVertices > 0)
			{
				...
				m_pd3dBoneWeightBuffer = d3dUtil::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf4BoneWeights, sizeof(XMFLOAT4) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dBoneWeightUploadBuffer);
